<script lang="ts">
	 ...
	 */
	let { variant }: { variant: 'contact' | 'scan' } = $props();
	const heading = $derived(variant === 'scan' ? 'Vraag je scan aan' : 'Stuur je vraag door');
	const submit = $derived(variant === 'scan' ? 'Vraag je scan aan' : 'Verstuur je vraag');
</script>
